...
//ON RELATION role    
    final SOtm userRoles = new SOtm("userRoles","userRole","role");                    
    dcm.addSOtm(userRoles); 
//ON RELATION role    
    final SOtm entityPermisionRoles = new SOtm("entityPermisionRoles","entityPermisionRole","role");                    
...
//Used to map error on index to source property
    insertMapFields.put("role.role_uidx_pname","pname");  
...
public Single<Map<String, Object>>  save(Role role) {
    ...
    public Single<Map<String,Object>> doList(final ObjForQuery objForQuery) {
        return doList000("role", objForQuery);
