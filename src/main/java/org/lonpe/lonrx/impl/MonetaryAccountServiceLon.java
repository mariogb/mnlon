...
//ON RELATION outAccount    
    final SOtm paymentOuts = new SOtm("paymentOuts","paymentOut","outAccount");                    
    dcm.addSOtm(paymentOuts); 
//ON RELATION inAccount    
    final SOtm paymentIns = new SOtm("paymentIns","paymentIn","inAccount");                    
    dcm.addSOtm(paymentIns); 
//ON RELATION monetaryAccount    
    final SOtm paymentOutForms = new SOtm("paymentOutForms","paymentOutForm","monetaryAccount");                    
    dcm.addSOtm(paymentOutForms); 
//ON RELATION monetaryAccount    
    final SOtm paymentInForms = new SOtm("paymentInForms","paymentInForm","monetaryAccount");                    
    dcm.addSOtm(paymentInForms); 
...
public Single<Map<String, Object>>  save(MonetaryAccount monetaryAccount) {
    ...
    public Single<Map<String,Object>> doList(final ObjForQuery objForQuery) {
        return doList000("monetary_account", objForQuery);
