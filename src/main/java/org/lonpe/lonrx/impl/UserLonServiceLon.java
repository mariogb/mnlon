...
public class UserLonServiceLon extends AbstractLon<UserLon> implements IServiceLon<UserLon>{  
    ...
    dcm.addSProperty(pkey);             //Create property email   
        ...
        final SProperty email = new SProperty("email", STRING,true,true);   
...
//PC
    ...
    dcm.addSProperty(typeLon);             //Create property username   
        ...
        final SProperty username = new SProperty("username", STRING,true,true);   
...
//ON RELATION userLon    
    final SOtm departamentUserLons = new SOtm("departamentUserLons","departamentUserLon","userLon");                    
    dcm.addSOtm(departamentUserLons); 
//ON RELATION userLon    
    final SOtm programUserLons = new SOtm("programUserLons","programUserLon","userLon");                    
    dcm.addSOtm(programUserLons); 
//ON RELATION userLon    
    final SOtm baseUserLons = new SOtm("baseUserLons","baseUserLon","userLon");                    
    dcm.addSOtm(baseUserLons); 
//ON RELATION userAutor    
    final SOtm sales = new SOtm("sales","sale","userAutor");                    
    dcm.addSOtm(sales); 
//ON RELATION userAutor    
    final SOtm purchases = new SOtm("purchases","purchase","userAutor");                    
    dcm.addSOtm(purchases); 
//ON RELATION userLon    
    final SOtm userRoles = new SOtm("userRoles","userRole","userLon");                    
    dcm.addSOtm(userRoles); 
//ON RELATION userLon    
    final SOtm userThirdPersons = new SOtm("userThirdPersons","userThirdPerson","userLon");                    
...
//Used to map error on index to source property
    insertMapFields.put("user_lon.user_lon_uidx_email","email");  
...
//Used to map error on index to source property
    insertMapFields.put("user_lon.user_lon_uidx_username","username");  
...
public Single<Map<String, Object>>  save(UserLon userLon) {
    ...
    public Single<Map<String,Object>> doList(final ObjForQuery objForQuery) {
        return doList000("user_lon", objForQuery);
